...
#include <stdio.h>
#include <stdbool.h>
#include "analog-sdk-common-plugin.h"
...
typedef void(*device_event)(ASDK_DeviceEventType, ASDK_DeviceInfo*);
...
ANALOGSDK_API int _read_full_buffer(uint16_t code_buffer[], float analog_buffer[], int len, ASDK_DeviceID device);
ANALOGSDK_API int _device_info(ASDK_DeviceInfo* buffer[], int len);
ANALOGSDK_API float read_analog(uint16_t code, ASDK_DeviceID device);
